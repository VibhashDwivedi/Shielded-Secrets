...
const VideoEncoder = () => {
  ...
  const encoding = useFormik({
    ...
    onSubmit: async values => {
      ...
      setEncodin(true);
      try {
        const response = await axios.post('http://localhost:5000/encode_vid', formData, {
          headers: {
            'Content-Type': 'multipart/form-data'
          }
        });
...
{uploading ? (
  ...
  <button onClick={handleVideoUpload} className="btn btn-primary rounded-0 mb-3">
    <i className="fa fa-upload"></i> Upload Video
            ...
            {totalFrames &&<div className="  mb-3">
            <label className="form-label title">Total number of Frames in Video:</label> <p className='title text-info-emphasis' style={{fontWeight:'900'}}> {totalFrames}</p> </div>}
           
            <form className='form-group' onSubmit={encoding.handleSubmit}>
            <div className="mb-3">
            <label className="form-label title">Select a Frame:</label>
            {encoding.touched.frameNumber && encoding.errors.frameNumber && <div className="alert alert-danger mt-2 p-2 rounded-0 fw-bold text-danger">{encoding.errors.frameNumber}</div>}
              <input autoComplete='off'  className="form-control rounded-0" type="number" name="frameNumber" onChange={encoding.handleChange} value={encoding.values.frameNumber} placeholder="Frame number" />
        
              </div>
              <div className="mb-3">
              <label className="form-label title">Secret Message:</label>
              {encoding.touched.data && encoding.errors.data && <div className="alert alert-danger mt-2 p-2 rounded-0 fw-bold text-danger">{encoding.errors.data}</div>}
              <input  autoComplete='off' className="form-control rounded-0" type="text" name="data" onChange={encoding.handleChange} value={encoding.values.data} placeholder="Data" />
              </div>
              <div className="mb-3">
              <label className="form-label title">Key:</label>
              {encoding.touched.key && encoding.errors.key && <div className="alert alert-danger mt-2 p-2 rounded-0 fw-bold text-danger" >{encoding.errors.key}</div>}
              <input type="password" autoComplete='off' className="form-control rounded-0"  name="key" onChange={encoding.handleChange} value={encoding.values.key} placeholder="Key" />
            ...
            <div className='d-block'>
              <label className="form-label title mt-2">Encoded Video:</label>
              <div>
              <video src={encodedVideo} controls height='270' width='350' className="img-thumbnail  rounded-0" />
              </div>
          ...
          <div className="container">
            <div className="card border-0">
              <div className="card-body bg-info-subtle">
                ...
                <p className='mb-4 para'>Follow these steps to encode your message in a video file:</p>
                <ul>
                  <li className='para text-start'>Select a video file into which you wish to embed your secret message.</li>
                  <li className='para text-start'>Click on the 'Upload Video' button to upload the selected video.</li>
                  <li className='para text-start'>Input the frame number where you wish to embed your message.</li>
                  <li className='para text-start'>Input the confidential message that you wish to embed within the video file.</li>
                  <li className='para text-start'>Input the cryptography key for the encoding process.</li>
                  <li className='para text-start'>Click on the 'Encode' button.</li>
                  <li className='para text-start'>The encoded video will be displayed below.</li>
                </ul>
              </div>
            </div>
          </div>
